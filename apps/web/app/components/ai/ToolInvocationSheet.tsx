...
export function ToolInvocationSheet({
  ...
  const renderToolContent = () => {
    switch (toolName) {
      case "fetchLyrics":
        return (
          <FetchLyricsResult state={toolState} args={args} result={result} />
        );
      case "verseReference":
        return (
      ...
      case "comprehensiveSongAnalysis":
        return (
          <SongAnalysisCard state={toolState} args={args} result={result} />
        );
      default:
        return (
          <GenericToolResult state={toolState} args={args} result={result} />
        );
    }
  };
          ...
          <Sheet.Content className="max-w-[700px] mx-auto bg-background dark:bg-[#191919] rounded-t-[24px] shadow-xl max-h-[85vh] overflow-y-auto border-t border-border/50 dark:border-[rgba(58,58,58,0.7)]">
            ...
            <div className="p-5 mb-2 border-b border-border/50 dark:border-[rgba(58,58,58,0.7)]">
              <div className="flex items-center justify-between">
                ...
                <Sheet.Trigger
                  action="dismiss"
                  className="p-2 rounded-full hover:bg-secondary/50 dark:hover:bg-[#3A3A3A] transition-colors"
                >
            ...
            {/* Content */}
            <div className="px-4 pb-8">{renderToolContent()}</div>
          </Sheet.Content>
        </Sheet.View>
      </Sheet.Portal>
    </Sheet.Root>
          ...
          <div className="text-sm text-foreground dark:text-white/80 bg-secondary/50 dark:bg-black/30 p-4 rounded-md overflow-x-auto border border-border/50 dark:border-[rgba(58,58,58,0.5)]">
            {Object.entries(args).map(([key, value]) => (
              <div key={key} className="mb-3 last:mb-0">
                <span className="text-muted-foreground dark:text-white/60 font-medium">
                  {key}:
                </span>{" "}
